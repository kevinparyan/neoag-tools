...
def write_peptide(fo, smuts, clone, wt, mt, mstr, idx,
    ...
    for index, m in smuts.iterrows():
        ...
        if start >= mstr_len or end >= mstr_len:
            warnings.warn(f"Skipping peptide for mutation {m.get('Protein_Change', 'N/A')} in {name}/{clone} transcript ID: {txid}, Hugo Symbol: {m.get('Hugo_Symbol', 'N/A')}, Variant_Classification: {m.get('Variant_Classification', 'N/A')}." f"Coordinates ({start}, {end}) out of bounds for mstr length {mstr_len}.")
            continue
